...
//     EnableTorque, Mode, unLockEprom, LockEprom and SyncWriteSpe's per-servo ACC write each burn
//     a full IOTimeOut when they are not acked
...
struct FakeServo
{
  ...
  int requests = 0;               // every non-broadcast request whose ID field is this servo
...
private:
  ...
  void run()
  {
    while (!stop_.load()) {
      ...
      if (ready == 0) {
        // nothing queued for a whole poll window; a half-parsed frame is not silence
        if (pending_.empty()) {
